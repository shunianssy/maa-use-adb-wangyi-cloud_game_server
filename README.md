...
-   **一键长草**：开始唤醒、领取奖励、自动公招、基建换班、每周剿灭、库存保持、理智作战、信用收支，按依赖顺序自动排队执行；勾选「开始唤醒」时若云游戏未连接会自动先启动云游戏；单个任务内子任务出错达到 5 次时自动跳过该任务并继续后续任务。
-   **库存保持**：勾选后先扫描仓库（MaaCore 仓库识别），按目标库存计算缺口并规划下一次理智作战（指定关卡 + 掉落停止条件、次数 AUTO，刷够缺口或理智耗尽即停），界面显示"下次打什么、哪一关"；优先级：每周剿灭 > 库存保持 > 理智作战配置。预设保持项：低级芯片（全职业，默认 20）、高级芯片组（全职业，默认 20）、采购凭证/红票（默认 20）、技巧概要·卷3（默认 200），目标数量可修改。资源关卡按开放日轮换（芯片本 PR-A/B/C/D、AP-5、CA-5），规划时自动跳过当天未开放的关卡并在日志提示下次开放日，避免 MAA 导航到关闭关卡而卡住。
...
-   **右侧栏（设置）**
    ...
    -   任务开关：开始唤醒、自动公招、基建换班、理智作战、库存保持、信用收支、领取奖励。勾选「开始唤醒」后点「开始执行」，若云游戏尚未连接会自动先启动云游戏并等待画面就绪。
    -   作战设置：作战关卡（留空=识别当前/上次）、次数（数字 / AUTO 刷完当前理智自动停；代理倍率恒为 AUTO）、理智药（关 / AUTO / 指定数量）、每周剿灭（AUTO 刷满止 / 固定场次；**默认不勾选**，需手动启用）。
    -   库存保持设置：勾选保持项（低级芯片全职业 / 高级芯片组全职业 / 采购凭证红票 / 技巧概要·卷3）并设置目标数量；勾选后执行到该任务时先扫描仓库，按缺口规划并自动刷对应关卡（次数 AUTO、刷够缺口即停），下方实时显示「下次理智作战: 关卡 · 物品 缺 N」；优先级：每周剿灭 > 库存保持 > 理智作战配置。
    -   日志里理智作战与每周剿灭都显示为 `中文名(Fight)`（如 `每周剿灭(Fight)`）——两者在 MaaCore 侧复用同一 `Fight` 任务类型，靠中文名区分来源；剿灭在本周合成玉已达上限时会直接跳过，不进关卡属正常。
